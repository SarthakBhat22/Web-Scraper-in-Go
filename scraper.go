...
func startScraping(
	...
	log.Printf("Starting scraper with %d goroutines every %s duration", concurrency, timeBetweenRequest)
...
func scrapeFeed(db *database.Queries, wg *sync.WaitGroup, feed database.Feed) {
	...
	for _, item := range rssFeed.Channel.Item {
		...
		if err != nil {
			log.Printf("Failed to parse date %v: %v", item.PubDate, err)
			continue
		}
	...
	log.Printf("Fetched feed %s with %d posts", feed.Name, len(rssFeed.Channel.Item))
}
